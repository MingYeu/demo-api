...
export const getStaffList = async (query: StaffPaginationQuery) => {
    const { page, row, sortField, sortOrder, fullNameEmail, department, nationality, status } = query;
    ...
    if (status) {
        staffWhereOptions.active = status === 'active';
    }

    if (nationality) {
        staffWhereOptions.nationality = nationality;
    }

    if (department) {
        staffWhereOptions.department = {
            id: department,
        };
    }
...
export const createStaff = async (staffId: string, staffValue: Staff) => {
    ...
    if (existingUsername) {
        throw {
            status: 400,
            message: 'APIMessage:usernameAlreadyExists',
        };
    }
    ...
    if (existingEmail) {
        throw {
            status: 400,
            message: 'APIMessage:emailAlreadyExists',
        };
    }
    ...
    if (!staffDetail) {
        throw {
            status: 404,
            message: 'APIMessage:staffNotFound',
        };
    }
    ...
    if (department['superAdmin'] && !staffDetail.department['superAdmin']) {
        throw {
            status: 403,
            message: 'APIMessage:permissionDenied',
        };
    }
    ...
    //     throw {
    //         status: 400,
    //         message: 'APIMessage:Email not sent',
    //     };
    // }
...
export const updateStaffDepartment = async (myStaffId: string, staffId: string, departmentId: string) => {
    if (myStaffId === staffId) {
        throw {
            status: 400,
            message: 'APIMessage:cannotUpdateYourself',
        };
    }
...
export const deleteStaff = async (myStaffId: string, staffId: string) => {
    if (myStaffId === staffId) {
        throw {
            status: 400,
            message: 'APIMessage:cannotDeleteYourself',
        };
    }
    const staffDetail = await prisma.staff.findFirst({
        where: {
...
export const changeStaffStatus = async (myStaffId: string, staffId: string, status: boolean) => {
    if (myStaffId === staffId) {
        throw {
            status: 400,
            message: 'APIMessage:cannotDeactivateYourself',
        };
    }
    const userDetail = await prisma.staff.findFirst({
        where: {
    ...
    if (userDetail['password'] === null) {
        throw {
            status: 400,
            message: 'APIMessage:Cannot deactivate unverified user',
        };
    }

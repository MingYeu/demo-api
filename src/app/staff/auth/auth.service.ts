...
export const authenticateStaff = async (token: string, permission: string) => {
    ...
    if (!response.active) {
        throw {
            message: 'APIMessage:Staff disabled',
            status: 400,
            clearStaffCookie: true,
        };
    }
...
export const login = async (username: string, password: string) => {
    ...
    if (!loginResponse) {
        throw {
            status: 400,
            message: 'APIMessage:Incorrect email or password',
        };
    }
    ...
    if (!loginResponse.password) {
        throw {
            status: 400,
            message: 'APIMessage:Your account yet to verified',
        };
    }
    ...
    if (!loginResponse.active) {
        throw {
            status: 400,
            message: 'APIMessage:staff is not active',
            clearStaffCookie: true,
        };
    }
...
export const verifyToken = async (staffId: string, token: string) => {
    ...
    if (staff.token !== token) {
        throw {
            status: 400,
            message: 'APIMessage:Invalid Token',
        };
    }
    ...
    if (dayjs().isAfter(dayjs(staff.tokenExpiredAt))) {
        ...
        throw {
            status: 400,
            message: 'APIMessage:Token Expired, Verification email resent!',
...
export const requestResetPassword = async (email: string) => {
    ...
    if (dayjs().isAfter(dayjs(staff.tokenExpiredAt))) {
        throw {
            status: 400,
            message: 'APIMessage:Token Expired',
        };
    }
    ...
    //     throw {
    //         status: 400,
    //         message: 'APIMessage:Failed to send the email',
    //     };
    // }
